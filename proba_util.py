...
def dist_meanvar(D):
    mean = sum(a * pa for a, pa in D.items())
    variance = sum(pa * (a - mean) ** 2 for a, pa in D.items())
    return mean, variance

def law_substract(A,B):
    C = {}
    for (a,pa) in B.items():
        C[-a] = pa
    return law_convolution(A,C)

def PRINT_DFR_HEAD():
    print("--------------------------------------------------------------")
    print("%-20s" % "name", "%-15s" % "method", "%-10s" % "log2(DFR)")
    print("--------------------------------------------------------------")

def PRINT_DFR_RESULT(name, method, dfr):
    print("%-20s" % name, "%-15s" % method, "%.2f" % dfr)
